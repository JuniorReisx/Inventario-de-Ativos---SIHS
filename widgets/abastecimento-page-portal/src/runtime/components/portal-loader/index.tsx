...
export default function PortalLoader (props: {
  ...
  const mark = (
    <div
      className={`portal-loader-agua${props.compact ? ' is-compact' : ''}`}
      role="status"
      aria-live="polite"
      aria-busy="true"
      aria-label={label}
    >
      ...
      {!props.compact
        ? <span className="portal-loader-agua__label">{label}</span>
        : null}
    </div>
  )
  if (!props.overlay) return mark
  return <div className="portal-loader-agua-overlay">{mark}</div>
}
