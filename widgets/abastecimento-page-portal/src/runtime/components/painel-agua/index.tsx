...
function loadScript (src: string): Promise<void> {
  ...
  const promise = new Promise<void>((resolve, reject) => {
    const existing = document.querySelector(`script[data-sihs-agua="${src}"]`) as HTMLScriptElement | null
    if (existing) {
...
const PainelAgua = ({ folderUrl }: { folderUrl: string }) => {
  ...
  React.useLayoutEffect(() => {
    ...
    const run = async () => {
      try {
        setErrorMessage('')
        // pts-data é opcional (aglomerados); não deve derrubar o painel
        const ptsPromise = loadScript(assetUrl(folderUrl, 'pts-data.js')).catch((error) => {
          console.warn('[abastecimento-page] pts-data.js indisponível:', error)
        })

        const prepared = await prepareAbastecimentoMap(mapNode)
        if (cancelled) {
        ...
        // Setores em background — KPIs já visíveis; bloco urbano/rural atualiza depois
        if (prepared.setoresLayer && !cancelled) {
          void loadSetoresUrbanoRural(prepared.setoresLayer, geo)
            ...
            .catch((error) => {
              console.warn('[abastecimento-page] setores urbano/rural:', error)
              if (cancelled) return
        ...
        console.error('[abastecimento-page] Falha ao ligar o web map:', error)
        if (!cancelled) {
  ...
  return (
    <div className={`abas-painel${status !== 'ready' ? ' is-booting' : ''}`} ref={rootRef}>
      {status !== 'ready' && status !== 'error'
        ? <PortalLoader overlay folderUrl={folderUrl} label="Carregando mapa e indicadores" />
        : null}
      {status === 'error'
        ? (
          <p className="abas-painel__error">
            Não foi possível carregar o web map de abastecimento.
            {errorMessage ? <span className="abas-painel__error-detail"> {errorMessage}</span> : null}
          </p>
          )
        : null}
      ...
      <div className="panel-shell">
                  ...
                  <div className="controls">
                    ...
                    <div className="filter-block">
                      ...
                      <div className="search-box">
                        <input type="text" id="muniSearch-agua" list="muniList-agua" placeholder="Consultar município..." />
                        <datalist id="muniList-agua"></datalist>
            ...
            <div className="data-grid">
              <div className="panel area-comp">
                <div className="panel-header">
                  <span>Formas de abastecimento</span>
                </div>
                <div className="panel-body">
                  <div id="compChart-agua"></div>
                </div>
              </div>
        ...
        <footer className="meta-footer">
          ...
          <span className="meta-label">Município</span><b>DPA Indicadores · SIDRA 6803</b>
          <span className="meta-dot">·</span>
