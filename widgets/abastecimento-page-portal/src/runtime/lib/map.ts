...
/** Web map ABASTECIMENTO - Inventário de Infraestrutura Hídrica e Saneamento */
...
export function resolveField (layer: any, ...candidates: string[]): string {
  ...
  // Evita devolver nome inexistente (quebra outFields na query)
  if (fields.length) {
    for (const want of candidates) {
      const w = normalizeText(want)
...
const POLY_OUTLINE = [7, 28, 51, 220]
const SEMI_OUTLINE = [27, 95, 160, 255]
...
async function querySemiGeometries (layer: any): Promise<any[]> {
  ...
  try {
    ...
    console.warn('[abastecimento] geometria da Região Semiárida:', error)
    return []
  }
}
...
async function queryDpaSemiMunicipioKeys (dpaLayer: any): Promise<{ codes: string[], names: string[] }> {
  ...
  while (true) {
    ...
    try {
      ...
      console.warn('[abastecimento] queryDpaSemiMunicipioKeys falhou:', error)
      break
    }
...
export async function querySemiMunicipioKeys (layer: any): Promise<{ codes: string[], names: string[] }> {
  ...
  await layer.load?.()
  // Só nomes reais de município — evita match parcial em campo "mun" (contagem agregada)
  ...
  while (true) {
    ...
    try {
      ...
      console.warn('[abastecimento] querySemiMunicipioKeys falhou:', error)
      break
    }
    const page = result?.features || []
    // Camada regional (1 polígono agregado): não há municípios individuais
...
async function querySemiCodesByIntersection (
  ...
  for (const geometry of semiGeometries.slice(0, 5)) {
    try {
      ...
      console.warn('[abastecimento] interseção Semiárido×DPA:', error)
    }
  }
  return { codes, names }
}
...
async function queryMunicipioAtPoint (layer: any, mapPoint: any): Promise<any | null> {
  ...
  try {
    ...
    console.warn('[abastecimento] queryMunicipioAtPoint falhou:', error)
    return null
  } finally {
...
async function resolveMunicipioGraphic (view: any, layer: any, event: any): Promise<any | null> {
  ...
  // 1) hitTest amplo (qualquer camada com atributos de município)
  try {
    ...
    // hit na camada alvo, mas sem atributos úteis → completa via query
  ...
  // 2) consulta espacial na camada DPA/PDA
  if (mapPoint) {
...
export function createMapApi (view: any, layer: any, options: {
  ...
  const zoomToWhere = async (where: string, targetLayer: any = layer, expand = 1.12, duration = 700) => {
    ...
    try {
      ...
      console.warn('[abastecimento] zoom falhou:', error)
      return false
    }
  }
  ...
  const selectSetorGraphic = async (graphic: any) => {
    ...
    if (where) {
      try {
        sl.featureEffect = {
          filter: { where },
          includedEffect: 'drop-shadow(0px, 0px, 16px, #2fb6dd) brightness(1.28) saturate(1.35)',
  ...
  const syncOverlayLayers = (state: PainelMapState) => {
    ...
    // Destaque opcional só se já houver geometria em cache (não baixa no boot)
    ...
    onSelect (handler) {
      const clickHandle = view.on('click', async (event: any) => {
        try {
          ...
          console.warn('[abastecimento] clique no mapa:', error)
        }
      })
...
export async function prepareAbastecimentoMap (container: HTMLElement): Promise<{
  view: any
  layer: any
  webMap: any
  setoresLayer: any | null
  ...
  try {
    // Garante atributos completos no hitTest (sem isso nome/código vêm vazios)
  ...
  if (semiLayer) {
    ...
    try {
      ...
      // Geometria só para destaque — lazy no 1º Ativar (ver syncOverlayLayers)
    } catch (error) {
      console.warn('[abastecimento] chaves da Região Semiárida_BA:', error)
    }
  }
