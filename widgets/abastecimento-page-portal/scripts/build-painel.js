...
const outCss = path.join(root, 'src', 'runtime', 'components', 'painel-agua', 'style.css')
...
export function initPainelAgua (root, GEO, PTS_DATA) {
  if (!root || !GEO) return () => {}
...
function wrapCss (source) {
  const prefixed = transformCss(source, '.abas-painel')
  return `/* Generated from css/painel.css — scoped to .abas-painel */\n${prefixed}
