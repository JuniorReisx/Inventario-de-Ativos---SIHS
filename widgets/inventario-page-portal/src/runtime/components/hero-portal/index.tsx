...
const HeroPortal = () => {
  ...
  React.useEffect(() => {
    ...
    const drawStatic = () => {
      ...
      g.addColorStop(0, '#001824')
      g.addColorStop(0.5, '#002a3d')
      g.addColorStop(1, '#0a5c66')
      ctx.fillStyle = g
      ctx.fillRect(0, 0, width, height)
      drawWave(height * 0.58, 18, 0.008, 0, 'rgba(26, 168, 200, 0.22)', 0)
      drawWave(height * 0.68, 22, 0.01, 0, 'rgba(10, 127, 168, 0.28)', 1.2)
      drawWave(height * 0.78, 16, 0.012, 0, 'rgba(0, 34, 49, 0.4)', 2.1)
    }
    ...
    const drawFrame = () => {
      ...
      bloom.addColorStop(0, 'rgba(47, 196, 255, 0.48)')
      bloom.addColorStop(0.35, 'rgba(26, 168, 200, 0.18)')
      ...
      bloom2.addColorStop(0, 'rgba(94, 200, 222, 0.14)')
      bloom2.addColorStop(1, 'rgba(94, 200, 222, 0)')
      ctx.fillStyle = bloom2
      ctx.fillRect(0, 0, width, height)

      const parallax = (pointerX - 0.5) * 10
      drawWave(height * 0.5 + parallax * 0.2, 22, 0.0072, 0.95, 'rgba(94, 200, 222, 0.14)', 0)
      drawWave(height * 0.6 + parallax * 0.35, 28, 0.009, 1.2, 'rgba(26, 168, 200, 0.2)', 1.4)
      drawWave(height * 0.7 + parallax * 0.5, 20, 0.011, 0.9, 'rgba(10, 127, 168, 0.26)', 2.6)
      drawWave(height * 0.8 + parallax * 0.65, 15, 0.013, 1.1, 'rgba(0, 34, 49, 0.42)', 0.7)

      for (let i = 0; i < 5; i++) {
        const y0 = height * (0.22 + i * 0.07)
        ctx.beginPath()
        ctx.strokeStyle = `rgba(34, 199, 230, ${0.08 + i * 0.015})`
        ctx.lineWidth = 1
      ...
      for (const p of particles) {
        ...
        ctx.beginPath()
        ctx.fillStyle = `rgba(94, 200, 222, ${a})`
    ...
    <div className="hero-portal" ref={rootRef}>
      ...
      >
        <canvas
          ...
          aria-hidden="true"
        />
        <div className="hero-portal__scrim" aria-hidden="true" />
        <div className="hero-portal__sweep" aria-hidden="true" />
        <div className="hero-portal__flow" aria-hidden="true" />

        <div className="hero-portal__inner">
          <div className="hero-portal__eyebrow">
