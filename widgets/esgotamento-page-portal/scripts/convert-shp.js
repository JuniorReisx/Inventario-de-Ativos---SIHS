...
async function main() {
  ...
  const assetsJs = path.join(ROOT, 'src', 'runtime', 'assets', 'geo-data.js');
  fs.mkdirSync(path.dirname(jsonPath), { recursive: true });
  fs.mkdirSync(path.dirname(assetsJs), { recursive: true });
  const json = JSON.stringify(geo);
  fs.writeFileSync(jsonPath, json);
  const js = 'window.GEO_DATA_ESGOTO = ' + json + ';\n';
  fs.writeFileSync(jsPath, js);
  fs.writeFileSync(assetsJs, js);
